...
export async function POST(request:Request){
 ...
 try{const db=await getRawDb();await db.prepare('INSERT INTO interests (id,name,email,role,organization,message,consent_version,created_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(email) DO NOTHING').bind(crypto.randomUUID(),p.name,p.email,p.role,p.organization,p.message,'labs-interest-v1',new Date().toISOString()).run();return Response.json({ok:true},{status:201,headers:{'Cache-Control':'no-store'}})}catch{console.error('Interest registration storage unavailable');return Response.json({error:'We could not save your interest. Your details are still here; please try again shortly.'},{status:503})}
}
